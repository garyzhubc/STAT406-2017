...
I will now illustrate a possibly more efficient to use bagging, namely
to store the $N$ trees (rather than their predictions on a given data set).
In this way I can re-use the ensamble in any future data set, without
having to re-train the elements of the *bag*. Since the idea is 
the same, I will just do it for when we tried a bag of 100 trees. 
To simplify the comparison between this way of implementing
baggin and the one I used above, I will re-create
the first training / test split
...
dat.tr <- Boston[ -ii, ]
```
Now I create a `list` of 100 (empty) elements, each entry in this 
list will later store a tree:
```{r bag1000.alt, fig.width=5, fig.height=5, message=FALSE, warning=TRUE}
N <- 100
mybag <- vector('list', N)
```
Now, we train the $N$ trees as before, and store them in the `list` (without
...
```
In order to obtain the predictions of each tree in my bag on the
test set, I could either:
...
The later is much more elegant and
compact, and both give exactly the same results. If we use the **first approach**
