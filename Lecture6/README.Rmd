...
Of course we can also try to estimate the MSPE of each solution (on a finite
grid of values of the penalty parameter) and select one estimator with
good prediction properties. If any of the 
corresponding estimated regression coefficients are exactly zero it
is commonly said that those explanatory variables were not selected to
enter the model. 
