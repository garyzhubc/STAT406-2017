...
methodology and to stress the point that these basis approaches are in fact nothing
...
is the matrix that has each of the 5 basis functions *f_1*, *f_2*, ..., *f_5* evaluated 
...
for *j=1*, ..., *k*, and *i=1*, ..., *n*. In the code below we use / abuse *recycling* rules 
used by `R` when operating with vectors and arrays (can you spot it?)
...
There are better (more stable) bases for the same linear space spanned by these spline functions. Here we
use the function `bs` to build a b-spline basis. Given the chosen knots and the degree of the splines
(linear, quadratic, cubic, etc.) the set of possible functions 
(a linear space, really) is the same regardless of the basis we use. As a consequence,
the estimated regression function should be identical for any basis we use (provided we do not run into
serious numerical issues). To illustrate this fact, we will use a B-spline basis with the same 5 knots
as above, and compare the estimated regression function with the one we obtained above 
...
As expected, both provide the same estimated regression function. Note that because we are 
using a set of linear splines, our estimated regression functions will always be piece-wise 
linear. To obtain smoother (e.g. differentiable, or even continuously differentiable)
regression estimators we can use higher-order splines. 

### Higher order splines (quadratic, cubic, etc.)

Here we directly use the function `bs` to evaluate the desired
spline basis on the observed values of the explanatory variable (in this case `range`). We use the arguments
`degree = 2` and `knots = kn` to indicate we want a 
quadratic spline basis with knots located at the elements of the vector `kn`. As before, we then simply use `lm` 
to estimate the coefficients, and overlay the estimated regression function over the data: 
...
A useful consequence of the fact that these regression estimators are in fact just linear regression estimators
...
for the fitted regression curve, as follows. We first fit and plot a quadratic 
...
space to look for a good regression estimator. Furthermore, 
we only need to select one parameter--the penalty term, which 
...
The function `smooth.spline` will compute an optimal value for the penalty term using 
leave-one-out cross-validation, if we set the argument `cv` to `TRUE`: 
...
Note that the optimal value found for `spar` is returned in the element 
