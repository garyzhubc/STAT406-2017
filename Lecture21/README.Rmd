...
Model-based clustering methods depend on a probabilistic (generative?) 
model that postulates that the distribution of the observed features
(over the whole population), typically as a mixture of several
...
one generally has to estimate the parameters that specify the 
...
surprising that the parameters are estimated using maximum
likelihood. The difficulty is that there are *n* unobserved
variables (the group labels) that also need to be estimated.
One can also think about this as having *n* missing observations,
and use the EM algorithm to perform maximum likelihood estimation
...
found on-line are either wrong, or wrongly derived. See
Lecture 22 for a more detailed discussion and a different
(and also very useful) application of the algorithm.
