...
this linear combination is not computed at once, but
iteratively, and only considering a specific class of
sets *A* (which ones?) As a result, the regression tree
is not the *global* optimal approximation by functions of
this type, but a good one that can be computed very rapidly.
...
To simplify the comparison of the predictions obtained
by trees and other regression estimators, we first randomly split the 
...
We now build a regression tree, leaving most of the arguments to their
default values. We specify the response and explanatory variables using
a `formula`, as usual, and the argument `method` to indicate we
need a regression tree (as opposed to a classification one, for example).
