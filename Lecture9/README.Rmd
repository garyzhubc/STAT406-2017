...
A different approach to constructing an estimated regression function is
based on recalling that the definition of the regression function 
...
that have **X close to c** (you can think of this as *working with the neighbours* of
**c**). This family of regression estimators are called
...
Probably the simplest kernel regression estimator would be to take
...
Note the gaps in the estimated regression function. Why do you think
they happened? We now increase the bandwidth from 50 to 60:
...
is fairly jagged, which is usually not how the true regression
function is expected to be. Can you explain why the above
regression estimator would look like this? 

As discussed in class, using a smoother kernel function 
results in a smoother estimated regression function. The plot
below uses the same bandwidth as before, but we use a 
standard gaussian density function as our kernel:
...
not be discussed here. Below we will use the `R` function `loess`
...
or variable) among the infinitely many possible ones, I expect the reader
...
function *locally*, using a linear function (corresponding to a 
Taylor expansion of order 1), or a quadratic function (expansion of
order 2), etc. We will illustrate this points using the `ethanol`
...
`E`. The span was arbitrarily set to 0.40.
...
Note how this regression estimator tends to miss the trends on the *tails* of the data
(e.g. for the smallest and largest observed values of `E`). A better fit
is obtained with a *locally linear* estimator, shown below in red, over the
...
This fit is an improvement from the previous one, but it is still unable to
capture the *peak* of the data (around `E` = 0.90). It is easy to see that
